...
//  SystemClock_Config();
  ...
  disable_WKUP();																		//we disable the WKUPs to avoid double starts
  ...
  if ((PWR->SR1 & (1<<8)) == (1<<8)){													//if SBF was HIGH

	  printf("Waking up\r\n");
	  PWR->SCR |= (1<<8);																//clear SBF (standby flag)
  ...
  GPIOA->MODER &= ~(1<<((5 * 2) + 1));													//PA5 output
